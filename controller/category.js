...
const addCategory = async (req, res, next) => {
  try {
    ...
    const user = await User.findById(_id).select(
      "-password -verificationCode -isVerifed"
    );
    if (!user) {
      res.code = 404;
...
const updateCategory = async (req, res, next) => {
  try {
    ...
    if (
      (categoryExist && String(category._id) !== String(categoryExist._id)) ||
...
const searchCategory = async (req, res, next) => {
  try {
    ...
    // Use size if provided, otherwise fall back to limit, otherwise default to 10
